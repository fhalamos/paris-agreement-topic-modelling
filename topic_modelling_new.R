...
docs <- docs %>% 
  ...
  #Remove "news construction network links" for 2019
...
#Remove empty rows
corpus_df_unique_t<-as.data.frame(corpus_df_unique_t[corpus_df_unique_t$text != "",])
colnames(corpus_df_unique_t)<-c("text")
...
#Last transform df back to corpus format
...
kable(head(frequency, 10), caption="Most common words")
...
df<-as.data.frame(terms(lda_model,5))
kable(df, caption = "Top 5 words in k=5 model")
